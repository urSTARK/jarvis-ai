...
const InputBar: React.FC<InputBarProps> = ({ onSendMessage, isListening, onToggleListen, isThinking, isShutdown, onWakeUp }) => {
  const [inputValue, setInputValue] = useState('');

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (inputValue.trim() && !isThinking && !isShutdown) {
    ...
    <div className="p-4 bg-neutral-900/50 backdrop-blur-sm border-t border-neutral-700/50">
      <form onSubmit={handleSubmit} className="flex items-center space-x-4">
        ...
        <button
          ...
          disabled={isThinking && !isShutdown}
          ...
          } ${isThinking && !isShutdown ? 'opacity-50 cursor-not-allowed' : 'transform hover:scale-110'}`}
          aria-label={isShutdown ? "Wake up Friday" : "Toggle voice listening"}
        >
          {isShutdown ? (
        ...
        <button
          type="submit"
          disabled={isThinking || !inputValue.trim() || isShutdown}
